...
```{r 1-five-components-gis, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
```{r 1-jogging-path, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
Spatial data can take many forms in a GIS. For the sake of our jogging example, you can imagine your jogging path could be represented as a line or a set of ordered points. In turn, your jogging path can be described by different qualities (day of the week, weather) and quantities (distance, speed, heart rate), which are called **attributes** (Figure \@ref(fig:1-jogging-path)). An important role of a GIS is to govern how data can interact and behave with other types of data. For example, we can calculate distance from a line, but not area, which is a special property of a polygon.
...
Geospatial data may be **captured** from remote sensing or digitized/scanned from other sources. Data can be **stored** in many different formats with different behaviours and rules and are kept in databases. Data can be queried (or selected) from a database using a standard query language. Once data are queried, they can then be **analysed** using operations like proximity, overlay, intersection and zonal statistics. A primary justification for investing resources into a GIS is to display and **output** data in the form of graphs, tables, charts, and maps.
...
```{r 1-niagara-falls-daguerreotype-1840, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
```{r 1-wright-flyer, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
```{r 1-jericho-air-station, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
Airplanes played a more significant role in World War II both as weapons and for reconnaissance. A new process for collecting aerial imagery was developed called **trimetrogon**, which involves capturing three photos at the same time: one directly down and two oblique images to the right and left of the airplane (Figure \@ref(fig:1-trimetrogon-airplane)). Even in the 1940's, much of Canada remained unmapped and unimaged, so the Royal Canadian Air Force contributed multiple photo squadrons with the task of mapping the vast country. One of those squadrons was No. 413 Photographic Squadron (Figure \@ref(fig:1-no-413-squadron)), which was based out of Canadian Forces Base Rockcliffe near Ottawa (formerly Rockliffe Air Station, the first air station established in Canada) and responsible for capturing the first aerial images of northern Canada. In the summer of 1950, No. 413 Photographic Squadron completed imaging the last gaps of land near Ellsemere Island. For the first time ever and more than 30 years after aerial surveys began, Canada finally had a self portrait from the sky. Today, a century after aerial surveying efforts began, the National Air Photo Library archives over six million air photos.

```{r 1-trimetrogon-airplane, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
```{r 1-no-413-squadron, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
...
(ref:1-cli-agriculture-capability-toronto-caption) Soil capability for agriculture from the Canada Land Inventory for the area surrounding Toronto, Canada. The majority of the best soils for farming in Canada (Class 1 - No significant limitations in use for crops) are found in southeastern Ontario. Queen's Printer, Public Domain. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/what-is-geomatics.html#fig:1-cli-agriculture-capability-toronto.

``` {r 1-cli-agriculture-capability-toronto, echo=FALSE, out.width="100%", fig.cap=fig_cap}
if (knitr:::is_latex_output()) {
  fig_cap = "(ref:1-cli-agriculture-capability-toronto-caption)"  
...
<!--chapter:end:01-what-is-geomatics.Rmd-->
...
```{r echo=FALSE, warning=FALSE}
yml_content <- yaml::read_yaml("chapterauthors.yml")
author <- yml_content[["mappingData"]][["author"]]
```
...
(ref:2-Eratosthenes-caption) Diagram showing how Eratosthenes estimated the circumference of Earth by observing the angle of a shadow that was cast about 800 km north of Syene in present-day Egypt. [@monniaux_illustration_2005], CC-BY-SA-4.0.

```{r 2-Eratosthenes, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:2-Eratosthenes-caption)"
} else { 
  fig_cap <- paste0("Diagram showing how Eratosthenes estimated the circumference of Earth by observing the angle of a shadow that was cast about 800 km north of Syene in present-day Egypt. [@monniaux_illustration_2005], CC-BY-SA-4.0.")
}
knitr::include_graphics("images/02-Eratosthenes.png")
```
<br/>
...
```{r 2-height, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Diagram for measuring height above a datum. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-height.png")
```
<br/>
...
```{r 2-geodetic-datum, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
    fig_cap <- paste0("Spherical geodetic datum. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-geodetic-datum.png")
```
<br/>
...
```{r 2-sphere-ellipsoid, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Sphere versus ellispoid. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-sphere-ellipsoid.png")
```
<br/>
...
```{r 2-tidal-datum, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Conceptual tidal datum for Canada. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-tidal-datum.png")
```
<br/>
...
```{r 2-H-h-N, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Orthometric Height (H) is the ellipsoidal height (h) less the geoid height (N). Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-H-h-N.png")
```
<br/>
...
```{r 2-geoid-tunnel, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Thought experiment showing where water would be at rest within a tunnel through the geoid due to the equipotential force of gravity (g). Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-geoid-tunnel.png")
```
<br/>
...
``` {r echo=FALSE, warning=FALSE, message=FALSE}
countries <- st_as_sfc("Data/02/canada.geojson", crs = st_crs("Data/02/canada.geojson"), GeoJSON = TRUE)
...
(ref:2-CGVD-leaflet-caption) The difference (in meters) between the Canadian Geodetic Vertical Datum of 2013 (CGVD2013) and the Canadian Geodetic Vertical Datum of 1928 (CGVD28). Pickell, CC-BY-SA-4.0. Data from Natural Resources Canada and licensed under the Open Government Licence - Canada: https://open.canada.ca/en/open-government-licence-canada. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/mapping-data.html#fig:2-CGVD-leaflet.

``` {r 2-CGVD-leaflet, echo=FALSE, out.width="100%", fig.cap=fig_cap}
if (knitr:::is_latex_output()) {
  fig_cap = "(ref:2-CGVD-leaflet-caption)"  
  knitr::include_graphics(here::here("images", "02-CGVD-leaflet.PNG"))
} else {
  fig_cap <- paste0("The difference (in meters) between the Canadian Geodetic Vertical Datum of 2013 (CGVD2013) and the Canadian Geodetic Vertical Datum of 1928 (CGVD28). Data from Natural Resources Canada and licensed under the <a href='",nrcanlicense,"'>Open Government Licence - Canada</a>. Pickell, CC-BY-SA-4.0.")
  m
}
```
...
```{r 2-geocentric-geodetic-latitude, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Geocentric versus geodetic latitude. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-geocentric-geodetic-latitude.png")
```
<br/>
...
(ref:2-geodesy-visualization-static-caption) Online geodesy tool for visualizing the differences between the sphere, ellipsoid, and geoid.  Floria Gu, CC-BY-SA-4.0. Click here to access the interactive tool: https://ubcemergingmedialab.github.io/geomatics-textbook/viz/geodesy-viz/

```{r 2-geodesy-visualization-static, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap = "(ref:2-geodesy-visualization-static-caption)"
} else { 
  fig_cap <- paste0("Online geodesy tool for visualizing the differences between the sphere, ellipsoid, and geoid. <a href='https://ubcemergingmedialab.github.io/geomatics-textbook/viz/geodesy-viz/'>Click here to access the interactive tool.</a> Floria Gu, CC-BY-SA-4.0.")
}
knitr::include_graphics("images/02-geodesy-visualization-static.png")
```
<br/>
...
```{r 2-reference-indicatrix, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Reference Tissot Indicatrix. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-reference-indicatrix.png")
```
<br/>
...
```{r 2-reference-equivalent, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Equivalent Tissot Indicatrix. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-equivalent-indicatrix.png")
```
<br/>

The indicatrix above in Figure \@ref(fig:2-reference-equivalent) is an example of an *equivalent* indicatrix, which has the following properties: $a>b$, $a×b=1$, and $Area=πab=π$. 
...
```{r 2-conformal-indicatrix, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Conformal Tissot Indicatrix. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-conformal-indicatrix.png")
```
<br/>
...
```{r 2-Mercator-map, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Mercator map projection with Tissot's Indicatrices in red. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-Mercator-map.png")
```
<br/>
...
```{r 2-UTM-13N-map, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Universal Transverse Mercator Zone 13 map projection with Tissot's Indicatrices in red. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-UTM-13N-map.png")
```
<br/>
...
```{r 2-Sinusoidal-map, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Sinusoidal map projection with Tissot's Indicatrices in red. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-Sinusoidal-map.png")
```
<br/>
...
```{r 2-BC-Albers-map, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("British Columbia Environment Albers map projection with Tissot's Indicatrices in red. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/02-BC-Albers-map.png")
```
<br/>
...
```{r 2-Azimuthal-map, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    ...
    knitr::include_graphics("images/02-Azimuthal-map.png")
```
<br/>
...
<!--chapter:end:02-mapping-data.Rmd-->
...
<!--chapter:end:03-types-of-data.Rmd-->
...
Government data are also increasingly published in data repositories, specific to the level of government (Figure \@ref(fig:4-data-network-model)). There are multiple levels to government, including municipalities (the smallest), sometimes counties (precincts, townships, or parishes), provinces (or states), and nations (the largest), each of which often has its own data repository. Centralized repositories are becoming increasingly common and connect open data from all levels of Government. The [Federated Research data repository](https://www.frdr-dfdr.ca/repo/) is an aggregation of Canadian open data repositories, including municipal, provincial, and academic repositories. It includes a map-based search for datasets with location information tied to their metadata. In the US, geospatial data from federal, municipal, and state government repositories are being consolidated under [Data.gov](https://www.data.gov/).
...
fig_cap <- paste0("The geographic levels of the Census of Canada include general units (applicable everywhere throughout Canada) and also an additional layer for urban areas only. Sutherland, CC-BY-SA-4.0.")
...
(ref:4-shifting-census-boundaries-caption) An example of how census boundaries have changed, showing changes in Census divisions for British Columbia from 1911 to 1986. Data from [@clark_historical_2016]. Sutherland, CC-BY-SA-4.0.

```{r 4-shifting-census-boundaries, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:4-shifting-census-boundaries-caption)"
} else { 
  fig_cap <- paste0("An example of how census boundaries have changed, showing changes in Census divisions for British Columbia from 1911 to 1986. Data from @clark_historical_2016. Sutherland, CC-BY-SA-4.0.")
}
knitr::include_graphics("images/04-shifting-census-boundaries.png")
```
<br/>
...
(ref:4-crooked-aerialphoto) Example of how a linear feature can appear crooked in an aerial photograph that has not yet been orthorectified due to relief displacement. [@dibiase_census_2014], CC-BY-4.0.

```{r 4-crooked-aerialphoto, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:4-crooked-aerialphoto-caption)"
} else { 
  fig_cap <- paste0("Example of how a linear feature can appear crooked in an aerial photograph that has not yet been orthorectified due to relief displacement. @dibiase_census_2014, CC-BY-4.0.")
}
knitr::include_graphics("images/04-crooked-aerialphoto.png")
```
<br/>
...
(ref:4-accessing-aerial-photos-caption) Example showing the availability of historical aerial photos in eastern Newfoundland at three time steps. Data from @natural_resources_canada_earth_nodate. Open Government License - Canada: https://open.canada.ca/en/open-government-licence-canada/.

```{r 4-accessing-aerial-photos, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:4-accessing-aerial-photos-caption)"
} else { 
...
knitr::include_graphics("images/04-accessing-aerial-photos.png")
```
<br/>
...
(ref:4-georeferencing-concept-caption) The need for georeferencing illustrated conceptually. Adapted from [@university_of_texas_libraries_intro_2021]. CC-BY-NC-2.0.

```{r 4-georeferencing-concept, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:4-georeferencing-concept-caption)"
} else { 
  fig_cap <- paste0("The need for georeferencing illustrated conceptually. Adapted from [@university_of_texas_libraries_intro_2021]. CC-BY-NC-2.0.")
}
...
```{r 4-control-points, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("A comparison of (A) an historical census map from 1931 with no graticule versus (B) a 1961 census map with graticule representing latitude and longitude. Panel C shows a close-up of the coordinate detail. Sutherland, CC-BY-SA-4.0.")
...
<!--chapter:end:04-collecting-and-editing-data.Rmd-->
...
if (knitr:::is_latex_output()) { 
  ...
  fig_cap <- paste0("Tabular and spatial data are related by a Relational Database Management System (RDBMS) in a Geographic Information System (GIS). Images of Douglas-Fir and Western Hemlock trees by [@canadian_forest_service_douglas-fir_2013], Canadian Forest Service, modified with permission.")
}
...
if (knitr:::is_latex_output()) { 
  ...
  fig_cap <- paste0("Cartesian product A×B of A (tuples) and B (attributes). Image by [@quartl_english_2012], CC BY-SA 3.0.")
}
...
```{r 5-boolean-and, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
...
```{r 5-boolean-or, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
...
```{r 5-boolean-not, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
...
```{r 5-boolean-xor, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
...
```{r 5-natural-join-3, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
...
```{r 5-outer-join, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
...
```{r 5-right-left-outer-join, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
...
```{r 5-right-left-outer-join-2, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
...
```{r 5-theta-join, echo=FALSE, warning=FALSE, message=FALSE}
knitr::kable(
...
```{r 5-case-study-2, echo=FALSE}
VancouverDA <- st_read("data/05/VancouverDA.shp", quiet=T)
VancouverDA <- data.frame(DAUID = VancouverDA$DAUID)
knitr::kable(
  head(VancouverDA), booktabs = TRUE,
  caption = 'An excerpt of the Vancouver DA polygon attributes.'
)
```
...
DA_CIMD <- st_read("data/05/VancouverDA_CIMD.shp", quiet=T)

DA_CIMD <- DA_CIMD %>%
  mutate_if(is.numeric,round, digits = 3)

DA_CIMD_proj <- st_transform(DA_CIMD, "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

pal <- colorNumeric(palette = "YlOrRd", domain = range(DA_CIMD$Situatio_1))

labels <- sprintf(
  "<strong>DAUID </strong> %s<br/><strong>Situational Vulnerabiltiy Score </strong> %g",
  DA_CIMD$DAUID, DA_CIMD$Situatio_1
) %>% lapply(htmltools::HTML)

m <- leaflet(DA_CIMD_proj) %>%
  addProviderTiles("CartoDB.Positron")%>%
  setView(lat = 49.255, lng = -123.153, zoom = 12) %>%
  addPolygons(fillColor = pal(DA_CIMD$Situatio_1), 
              weight = 0.6, 
              opacity = 1,
              color = "#c2c2c2",
              dashArray = NULL,
  ...
  addLegend(pal = pal, values = ~DA_CIMD$Situatio_1, opacity = 0.7, 
            title = "Situational Vulnerabiltiy Score",
            position = "bottomleft")
```

(ref:5-situational-vulnerability-leaflet-caption) An example of a choropleth map displaying the situational vulnerability scores in the City of Vancouver at DA level. Higher score represents the DA has higher situational vulnerable population e.g., population lacking a high school diploma, low-income population. Lam, CC-BY-SA-4.0. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/relational-databases.html#fig:5-situational-vulnerability-leaflet.

```{r 5-situational-vulnerability-leaflet, fig.cap = fig_cap, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  fig_cap <- "(ref:5-situational-vulnerability-leaflet-caption)"
  knitr::include_graphics(here::here("images", "05-situational-vulnerability-leaflet.PNG"))
} else {
  fig_cap <- paste0("An example of a choropleth map displaying the situational vulnerability scores in the City of Vancouver at DA level. Higher score represents the DA has higher situational vulnerable population e.g., population lacking a high school diploma, low-income population. Lam, CC-BY-SA-4.0.")
  m
}
```
<br/>
...
calculated <- st_read("data/05/Results.shp", quiet=T)
calculated.full <- head(calculated[c(1, 14, 15, 17, 16, 18, 21)])
colnames(calculated.full) <- c("DAUID","Aggregated scores", "Normalized aggregated scores", "Species count", "Street tree diversity", "Vegetation richness", "Vegetation diversity scores","geometry")
calculated.full <- calculated.full %>%
  mutate_if(is.numeric,round, digits = 3)
```

```{r 5-case-study-6, echo=FALSE}
calculated.full <- as.data.frame(calculated.full)
knitr::kable(
  calculated.full[,-8], booktabs = TRUE,
  caption = 'An excerpt of the attribute table after the joins and calculations.'
)
```

```{r 5-case-study-7, include=FALSE}
full.results <- st_read("data/05/Results.shp", quiet=T)
full.results_proj <- st_transform(full.results, "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
pal1 <- colorNumeric(palette = "Greens", domain = range(full.results_proj$veg_div_sc))
...
(ref:5-vegetation-diversity-leaflet-caption) An example of a choropleth map displaying vegetation diversity for each dissemination area in Vancouver. Lam, CC-BY-SA-4.0. Data from City of Vancouver and licensed under the Open Government License - Vancouver: https://opendata.vancouver.ca/pages/licence/. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/relational-databases.html#fig:5-vegetation-diversity-leaflet.

```{r 5-vegetation-diversity-leaflet, fig.cap = fig_cap, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  fig_cap <- "(ref:5-vegetation-diversity-leaflet-caption)"
  knitr::include_graphics(here::here("images", "05-vegetation-diversity-leaflet.PNG"))
} else {
  fig_cap <- paste0("An example of a choropleth map displaying vegetation diversity for each dissemination area in Vancouver. Data from City of Vancouver and licensed under the <a href='",covlicence,"'>Open Government License - Vancouver</a>. Lam, CC-BY-SA-4.0.")
  m1
}
```
<br/>
...
query.results <- calculated[calculated$norm_aggre >= 0.5 & calculated$veg_div_sc < 0.5,]
query.results <- query.results[,c(1,15,16,18,21)]
colnames(query.results) <- c("DAUID", "Normalized aggregated scores", "Street tree diversity", "Vegetation richness", "Vegetation diversity scores","geometry")
query.results <- query.results %>%
  mutate_if(is.numeric,round, digits = 3)
```

```{r 5-case-study-9, echo=FALSE}
query.results <- as.data.frame(query.results)
knitr::kable(
  query.results[,-6], booktabs = TRUE,
...
```{r echo=FALSE, warning=FALSE}
yml_content <- yaml::read_yaml("chapterauthors.yml")
author <- yml_content[["topologyGeocoding"]][["author"]]
```
...
``` {r 7-load-packages-and-setup, echo = FALSE, include = FALSE}
baseurl <- "https://ubc-geomatics-textbook.github.io/geomatics-textbook/"
canadalicense <- "https://open.canada.ca/en/open-government-licence-canada/"
...
fig_cap <- paste0("Grid showing all the combinations of how point, line and polygon geometries can interact. Pickell, CC-BY-SA-4.0.")
...
``` {r, 7-native-land-data-load, echo=F, message=F, warning=F, cache=T}
native_land <- st_as_sfc("Data/07/07-native_land_vancouver_island.geojson", crs = st_crs("Data/07/07-native_land_vancouver_island.geojson"), GeoJSON = TRUE)
labels <- native_land$Name
```

``` {r 7-native-land-leaflet-setup, cached=T, echo = FALSE, include = FALSE}
...
(ref:7-native-land-leaflet-caption1) Non-planar topology of 36 indigenous territories overlapping Vancouver Island, British Columbia. Data from [@native_land_native_nodate], CC0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html.

(ref:7-native-land-leaflet-caption2) Non-planar topology of 36 indigenous territories overlapping Vancouver Island, British Columbia. Data from [@native_land_native_nodate], CC0.

```{r 7-native-land-leaflet, fig.cap = fig_cap, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) { fig_cap ="(ref:7-native-land-leaflet-caption1)"
knitr::include_graphics("images/07-native-land-leaflet.png")
} else { fig_cap = "(ref:7-native-land-leaflet-caption2)"
m
}
```
...
``` {r echo=FALSE, warning=FALSE, message=FALSE}
countries <- st_read("data/02/world-countries.shp", quiet=T)
uscanada <- countries[countries$CNTRY_NAME == "Canada" | countries$CNTRY_NAME == "United States",]
uscanada_sf <- st_as_sf(uscanada)
uscanada_simp <- st_simplify(uscanada_sf, preserveTopology = FALSE, dTolerance = 1000)
```

``` {r echo=FALSE, warning=FALSE, message=FALSE}
pal <- colorFactor(palette = c("#a50026","#313695"), levels = c("Canada","United States"))
m = leaflet(uscanada_simp) %>%
  addTiles() %>%
  setView(lng = -94.0637905778631, lat = 48.64363272747508, zoom = 9) %>%
  addScaleBar(position = c("topleft")) %>%
  addPolygons(stroke = T, color=~pal(CNTRY_NAME), weight=1, smoothFactor = 0.3, fillOpacity = 0.8)
```

``` {r echo=FALSE, out.width="100%", fig.cap=fig_cap}
if (knitr:::is_latex_output()) { fig_cap <- paste0("Simplified boundary of Canada (red) and the United States (blue), centred at the Rainy River boundary between Minnesota and Ontario. Without preserving topology, the output results in illogical overlaps in some places and slivers in other places. Pickell, CC-BY-SA-4.0. Data from Natural Resources Canada and licensed under the Open Government Licence - Canada. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html")
  knitr::include_graphics("images/07-Canada-US-border-simplified-leaflet.PNG")
} else { fig_cap <- paste0("Simplified boundary of Canada (red) and the United States (blue), centred at the Rainy River boundary between Minnesota and Ontario. Without preserving topology, the output results in illogical overlaps in some places and slivers in other places. Pickell, CC-BY-SA-4.0. Data from Natural Resources Canada and licensed under the Open Government Licence - Canada.")
m
}
```
...
```{r, 7-dangle, fig.align = 'center', out.width = '75%', fig.cap=fig_cap, cached=T, echo = FALSE}
...
```{r, 7-intersection-A-B, fig.align = 'center', out.width = '75%', fig.cap=fig_cap, cached=T, echo = FALSE}
fig_cap <- paste0("Line A mapped with Line B in non-planar topology. Pickell, CC-BY-SA-4.0.")
Ax <- c(1,1,1)
Ay <- c(0,3,5)
Bx <- c(0,2,4)
By <- c(2,2,2)
...
```{r, 7-intersection-C-D-E-F, fig.align = 'center', out.width = '75%', fig.cap=fig_cap, cached=T, echo = FALSE}
...
```{r, 7-pseudonode, fig.align = 'center', out.width = '75%', fig.cap=fig_cap, cached=T, echo = FALSE}
fig_cap <- paste0("Lines A and B share a pseudonode at [3,5], indicated in red. Pickell, CC-BY-SA-4.0.")
Ax <- c(0,1,3)
Ay <- c(0,3,5)
Bx <- c(3,4,3)
By <- c(5,3,1)
...
```{r 7-dam-river-watershed, echo=FALSE, out.width = '50%', fig.cap=fig_cap}
...
haida <- st_as_sfc("Data/07/07-haida-gwaii-islands-names.geojson", crs = st_crs("Data/07/07-haida-gwaii-islands-names.geojson"), GeoJSON = TRUE)
labels2 <- haida$Island_Name
```

``` {r 7-haida-leaflet-setup2, echo=FALSE, warning=FALSE, message=FALSE, cached=T, echo = FALSE}
...
(ref:7-haida-gwaii-leaflet-caption) Singlepart geometry of the Haida Gwaii archipelago off the west coast of British Columbia, Canada. Hover over the islands to see the names. Polygon data from [@statistics_canada_statistics_nodate] and island placenames from [@natural_resources_canada_natural_nodate]. Open Government License - Canada: https://open.canada.ca/en/open-government-licence-canada/. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology.html#fig:7-haida-gwaii-leaflet.

```{r 7-haida-gwaii-leaflet, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:7-haida-gwaii-leaflet-caption)"
  knitr::include_graphics("images/07-Singlepart-geometry-of-the-Haida-Gwaii-archipelago-off-the-west-coast-of-British-Columbia-Canada-static.png")
} else { 
  fig_cap <- paste0("Singlepart geometry of the Haida Gwaii archipelago off the west coast of British Columbia, Canada. Hover over the islands to see the names. Polygon data from [@statistics_canada_statistics_nodate] and island placenames from [@natural_resources_canada_natural_nodate]. <a href='",canadalicense,"'>Open Government License - Canada</a>.")
m2
}
```
<br/>
...
``` {r 7-delaunay-triangulation-setup2, echo=FALSE, cached=T}
## Not run: creates image
## set.seed(14)
## x <- runif(50)
## y <- runif(50)
...
## png("/images/07-delaunay-triangulation.png",height=600,width=600)
...
if (knitr:::is_latex_output()) { 
  knitr::include_graphics("images/07-2d-alpha-hull-static.png")
} else { 
...
In this next section, a number of topologies are described that are important for 3D modeling and several examples are given using LiDAR (Light Detection and Ranging), which is the topic of [Chapter 15](https://ubc-geomatics-textbook.github.io/geomatics-textbook/LiDAR-acquisition-and-analysis.html). It is beyond the scope of this chapter to discuss the technology of LiDAR, so the reader is referred to [Chapter 15](https://ubc-geomatics-textbook.github.io/geomatics-textbook/LiDAR-acquisition-and-analysis.html) for a more in-depth discussion of LiDAR.
...
(ref:7-platonic-polyhedrons-caption) The five Platonic solids are examples of regular, convex polyhedrons and multipatch geometries. From left to right: tetrahedron (4 faces); hexahedron (6 faces); octahedron (8 faces); dodecahedron (12 faces); and icosahedron (20 faces). [@cyp_polyhedra_2005], CC-BY-SA-3.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology.html#7-platonic-polyhedrons.

```{r 7-platonic-polyhedrons, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:7-platonic-polyhedrons-caption)"
knitr::include_graphics("images/07-platonic-polyhedrons-static.png")
} else { 
  fig_cap <- paste0("The five Platonic solids are examples of regular, convex polyhedrons and multipatch geometries. From left to right: tetrahedron (4 faces); hexahedron (6 faces); octahedron (8 faces); dodecahedron (12 faces); and icosahedron (20 faces). [@cyp_polyhedra_2005], CC-BY-SA-3.0.")
...
```{r 7-setup-3d-alpha-hull-deciduous, echo=FALSE, cached=T, warning=F, include=F}
las <- lidR::readLAS("Data/07/07-deciduous-tree.las")
z <- las@data$Z
x <- las@data$X
y <- las@data$Y
...
(ref:7-3d-alpha-hull-deciduous-caption1) 3D concave alpha hull for a deciduous tree. The alpha hull was generated using alpha = 0.05. Data collected by Spencer Dakin Kuiper with a GeoSlam terrestrial laser scanner in Vancouver, Canada. Pickell, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html#fig:7-3d-alpha-hull-deciduous.

(ref:7-3d-alpha-hull-deciduous-caption2) 3D concave alpha hull for a deciduous tree. The alpha hull was generated using alpha = 0.05. Data collected by Spencer Dakin Kuiper with a GeoSlam terrestrial laser scanner in Vancouver, Canada. Pickell, CC-BY-SA-4.0.

```{r 7-3d-alpha-hull-deciduous, echo=FALSE, out.width = '75%', fig.cap=fig_cap, webgl = TRUE}
if (knitr:::is_latex_output()) { fig_cap = "(ref:7-3d-alpha-hull-deciduous-caption1)"
knitr::include_graphics("images/07-3d-alpha-hull-deciduous-static.png")
} else { fig_cap = "(ref:7-3d-alpha-hull-deciduous-caption2)"
plot3d(ahull.d,col="#2D5A27")
rglwidget(width = 600, height = 600, reuse=FALSE, elementId="7-3d-alpha-hull-deciduous")
}
```

``` {r 7-setup-3d-alpha-hull-conifer, echo=F, cached=T, warning=F, include=F}
las <- readLAS("Data/07/07-conifer-tree.las")
z <- las@data$Z-1156
x <- las@data$X-640600
y <- las@data$Y-5674900
xyz <- cbind(x,y,z)
ahull.c <- ashape3d(xyz,alpha=0.1)
```

(ref:7-3d-alpha-hull-conifer-caption1) 3D concave alpha hull for a coniferous tree. The alpha hull was generated using alpha = 0.05. Data collected by Spencer Dakin Kuiper with a GeoSlam terrestrial laser scanner in Vancouver, Canada. Pickell, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html#fig:7-3d-alpha-hull-conifer.

(ref:7-3d-alpha-hull-conifer-caption2) 3D concave alpha hull for a coniferous tree. The alpha hull was generated using alpha = 0.05. Data collected by Spencer Dakin Kuiper with a GeoSlam terrestrial laser scanner in Vancouver, Canada. Pickell, CC-BY-SA-4.0.

``` {r 7-3d-alpha-hull-conifer, echo=FALSE, out.width = '75%', fig.cap=fig_cap, webgl = TRUE}
if (knitr:::is_latex_output()) { fig_cap = "(ref:7-3d-alpha-hull-conifer-caption1)"
knitr::include_graphics("images/07-3d-alpha-hull-conifer-static.png")
} else { fig_cap = "(ref:7-3d-alpha-hull-deciduous-caption2)"
plot3d(ahull.c,xlim=c(40,64),ylim=c(21,45),col="#2D5A27")
rglwidget(width = 600, height = 600, reuse=FALSE, elementId="7-3d-alpha-hull-conifer")
}
```
...
(ref:7-neper-caption) 3D Voronoi tesselation of simulated polycrystals. [@quey_neper_nodate], GPL 3.0.

```{r 7-neper, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:7-neper-caption)"
} else { 
  fig_cap <- paste("3D Voronoi tesselation of simulated polycrystals. @quey_neper_nodate, GPL 3.0.")
}
...
2. Define topology.
3. How are Thiessen Polygons related to the Delaunay Triangulation?
4. How do you interpret a Voronoi diagram? Give an example for how you could use this layer to solve a spatial problem.
5. What are some uses of a multipart geometry?

## Practice Questions {-}

1. Draw a convex hull for the points below. Estimate where the centroid is for the polygon you drew.
2. Draw the Delaunay Triangulation for the points below.
3. Draw two lines from the points below that share a pseudonode.
4. Draw a polygon with a hole from the points below.

```{r 7-scatterplot, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
fig_cap <- paste("Ten random points. Pickell, CC-BY-SA-4.0.")
knitr::include_graphics("images/07-scatterplot.png")
```
...
<!--chapter:end:07-topology-and-geocoding.Rmd-->
...
```{r 9-load-packages-and-setup, include=FALSE, warning=FALSE}
baseurl <- "https://ubc-geomatics-textbook.github.io/geomatics-textbook/"
covlicence <- "https://opendata.vancouver.ca/pages/licence/"
knitr::opts_chunk$set(echo = TRUE)
library(DiagrammeR)
...
**Digital Vertical Models (DVM)** represent vertical heights and elevations of terrain and features. *Vertical* refers to the axis of geographic space that is orthogonal (i.e., perpendicular) to a vertical datum, such as a geoid or ellipsoid. Thus, a DVM may represent elevation of terrain or the height of a building. Both of these examples have specific meanings and calculations relative to the vertical datum, which we will explore in more detail in the following sections.

## Digital Elevation Models (DEM)

As the name suggests, a **Digital Elevation Model (DEM)** is a digital representation of elevation data or heights above a vertical datum. We call it a _model_ because all spatial data formats are inherently spatial data models. As is the case for all rasters, DEMs represent sampled data. That is, the elevations that are represented continuously in the raster DEM are in fact samples of elevation at the raster cell centres. For this reason, a DEM is a model that is simplifying elevation to some degree depending on the cell size. 
...
```{r 9-mount-assiniboine-dem, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Digital Elevation Model of Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Pickell, CC-BY-4.0.")
...
```{r 9-nelson-river-dem, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Digital Elevation Model of Nelson River pouring into Hudson Bay, Manitoba, Canada. Data from @earth_resources_observation_and_science_center_usgs_2018. Pickell, CC-BY-4.0.")
...
```{r 9-mount-assiniboine-slope, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Slope of Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Pickell, CC-BY-SA-4.0.")
...
```{r 9-nelson-river-slope, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Slope of Nelson River pouring into Hudson Bay, Manitoba, Canada. Data from @earth_resources_observation_and_science_center_usgs_2018. Pickell, CC-BY-SA-4.0.")
...
```{r 9-mount-assiniboine-aspect, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Aspect of Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Pickell, CC-BY-SA-4.0.")
...
```{r 9-nelson-river-aspect, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Aspect of Nelson River pouring into Hudson Bay, Manitoba, Canada. Data from @earth_resources_observation_and_science_center_usgs_2018. Pickell, CC-BY-SA-4.0.")
...
```{r 9-mount-assiniboine-nelson-river-heat-load-index, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Comparing Heat Load Index (HLI) for Mount Assiniboine and the Nelson River. Pickell, CC-BY-SA-4.0.")
...
(ref:9-mount-assiniboine-hillshade-animation-caption) Hillshade of Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Online version is animated across all azimuths from 0° to 350° by 10° intervals. Pickell, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/raster-analysis-and-terrain-modelling.html#fig:9-mount-assiniboine-hillshade-animation.

```{r 9-mount-assiniboine-hillshade-animation, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap = "(ref:9-mount-assiniboine-hillshade-animation-caption)"
  knitr::include_graphics("images/09-mount-assiniboine-hillshade-animation-static.png")
} else { 
  fig_cap <- paste0("Hillshade of Mount Assiniboine at the border of Alberta and British Columbia, Canada animated across all azimuths from 0° to 350° by 10° intervals. Pickell, CC-BY-SA-4.0.")
...
(ref:9-nelson-river-hillshade-animation-caption) Hillshade of Nelson River pouring into Hudson Bay, Manitoba, Canada. Online version is animated across azimuths from 0° to 350° by 10° intervals. Pickell, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/raster-analysis-and-terrain-modelling.html#fig:9-nelson-river-hillshade-animation.

```{r 9-nelson-river-hillshade-animation, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap = "(ref:9-nelson-river-hillshade-animation-caption)"
  knitr::include_graphics("images/09-nelson-river-hillshade-animation-static.png")
} else { 
  fig_cap <- paste0("Hillshade of Nelson River pouring into Hudson Bay, Manitoba, Canada animated across azimuths from 0° to 350° by 10° intervals. Data from @earth_resources_observation_and_science_center_usgs_2018. Pickell, CC-BY-SA-4.0.")
...
```{r 9-sink, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("A sink is shown in 3D for an array of 3x3 pixels. Pickell, CC-BY-SA-4.0.")
...
```{r 9-peak, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("A peak is shown in 3D for an array of 3x3 pixels. Pickell, CC-BY-SA-4.0.")
...
```{r 9-saddle, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("A saddle is shown in 3D for an array of 3x3 pixels. Pickell, CC-BY-SA-4.0.")
...
```{r 9-stacked-profile-planform, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Conceptual diagram showing surfaces of all combinations of profile and planform curvature. Pickell, CC-BY-SA-4.0.")
...
```{r 9-stacked-profile-planform-kernel, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Conceptual diagram showing 3x3 kernel examples of all combinations of profile and planform curvature. Pickell, CC-BY-SA-4.0.")
...
```{r 9-mount-assiniboine-profile-planform-curvature, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Profile and planform curvature for Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Pickell, CC-BY-SA-4.0.")
...
```{r 9-mount-assiniboine-nelson-river-topographic-position-index, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Comparing Topographic Position Index (TPI) for Mount Assiniboine and the Nelson River. Data from @natural_resources_canada_canadian_2015 and @maxar_precision3d_nodate, respectively. Pickell, CC-BY-SA-4.0.")
...
```{r 9-flow-direction-accumulation, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Flow direction shown in arrows and flow accumulation of upslope cells shown in numbers for the D8 flow direction algorithm. Pickell, CC-BY-SA-4.0.")
...
(ref:9-sink-fill-caption) Process of filling sinks in a DEM. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/raster-analysis-and-terrain-modelling.html#fig:9-sink-fill.

```{r 9-sink-fill, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:9-sink-fill-caption)"
  knitr::include_graphics("images/09-sink-fill-static.png")
} else { 
  fig_cap <- paste("Animated process of filling sinks in a DEM. Pickell, CC-BY-SA-4.0.")
...
```{r 9-mount-assiniboine-flow-dir-acc, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("D8 Flow direction and flow accumulation for Mount Assiniboine at the border of Alberta and British Columbia, Canada. Data from @natural_resources_canada_canadian_2015. Pickell, CC-BY-SA-4.0.")
...
```{r 9-Mattivi-et-al-2019-figure-1, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Conceptual figure showing how Topographic Wetness Index (TWI) is a function of (a) upslope area, (b) area and direction of local flow, and (c) the tangent of the local slope angle. Reproduced from @mattivi_twi_2019, CC-BY-4.0.")
...
```{r 9-fort-st-john-study-area, echo=FALSE, fig.cap=fig_cap, ouwidth="100%"}
...
if (knitr:::is_latex_output()) {
knitr::include_graphics(here::here("images", "09-fort-st-john-study-area.png"))
} else {
m
}
```
...
```{r 9-SAGA-Wetness, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("System for Automated Geoscientific Analyses (SAGA) Wetness Index across the Chowade Watershed. Melser, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Multi-resolution Valley Bottom Flatness (MRVBF) across the Chowade Watershed. Melser, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Cross Section where elevation is expressed through Multi-Resolution Valley Bottom Flatness (MRVBF). High values represent valley bottoms, and low values represent steep slopes. Melser, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Elevation Cross Section. Melser, CC-BY-SA-4.0.")
...
(ref:9-leaflet-map-layers-caption) Input layers and output prediction raster for the Chowade Watershed Management basin. Melser, CC-BY-SA-4.0. Data from @city_of_vancouver_notitle_2009 and licensed under the  under the Open Government License - Vancouver: https://opendata.vancouver.ca/pages/licence/. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/raster-analysis-and-terrain-modelling.html#fig:9-leaflet-map-layers.

```{r 9-leaflet-map-layers, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:9-leaflet-map-layers-caption)"
  knitr::include_graphics(here::here("images", "09-leaflet-map-layers.png"))
} else { 
  fig_cap <- paste0("Input layers and output prediction raster for the Chowade Watershed Management basin. Data from @city_of_vancouver_notitle_2009 and licensed under the  under the <a href='",covlicence,"'>Open Government License - Vancouver</a>. Melser, CC-BY-SA-4.0.")
  m2
}
```
...
```{r 9-baffin-island-mount-odin-1, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Anaglyph of Mount Odin on Baffin Island, Nunavut, Canada. Imagery by @maxar_precision3d_nodate. Pickell, CC-BY-SA-4.0.")
...
(ref:9-vancouver-building-height-extruded-caption) Animation showing building polygons in Vancouver, British Columbia extruded by a height attribute. Pickell, CC-BY-SA-4.0. Building data from @city_of_vancouver_notitle_2009, licensed under Open Government License - Vancouver: https://opendata.vancouver.ca/pages/licence/. Base map © @openstreetmap_notitle_nodate contributors, licensed under Open Data Commons Open Database License. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/raster-analysis-and-terrain-modelling.html#fig:9-vancouver-building-height-extruded.

```{r 9-vancouver-building-height-extruded, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:9-vancouver-building-height-extruded-caption)"
  knitr::include_graphics("images/09-vancouver-building-height-extruded-static.png")
} else { 
  fig_cap <- paste0("Animation showing building polygons in Vancouver, British Columbia extruded by a height attribute. Pickell, CC-BY-SA-4.0. Building data from @city_of_vancouver_notitle_2009, licensed under <a href='",covlicence,"'>Open Government License - Vancouver</a>. Base map © @openstreetmap_notitle_nodate contributors, licensed under Open Data Commons Open Database License.")
...
```{r 9-mount-logan-exaggeration, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Elevation exaggeration of Mount Logan elevation, Yukon Territory, Canada. Digital Elevation Model by @natural_resources_canada_canadian_2015. Pickell, CC-BY-SA-4.0.")
    knitr::include_graphics("images/09-mount-logan-exaggeration.png")
```

<!--chapter:end:09-raster-analysis-terrain-modelling.Rmd-->
...
2.  Distinguish different types of energy used in remote sensing
...
```{r 11-hermosilla-canada-landcover, width="100%", fig.cap = fig_cap, echo = FALSE}
...
Expressing extremely large or small numbers presents a challenge to both efficiency and accessibility that has existed likely since the creation of mathematics. Scientific notation presents a simple solution to this problem through simplifying numeric presentation to a value less than 10 that is raised to a particular power. Put simply, the decimal point of a large or small number is moved to make the smallest, single digit whole number. The number of places and direction that the decimal point moves is described by an associated power of 10. Equations \@ref(eq:SI-large) and \@ref(eq:SI-small) depict how large and small numbers are presented in scientific notation, respectively.
...
It is common to use mathematical operators to add, subtract, divide or multiply large numbers written in scientific notation. Addition and subtraction require the number with the smallest exponent to be altered to match the exponent of the largest number \@ref(eq:SI-add). The multiplication of two numbers written in scientific notation you simply multiply their coefficients and add their exponents \@ref(eq:SI-multiply). Division of two scientific numbers follows a similar structure, in which you divide the coefficients and subtract the exponents \@ref(eq:SI-divide).
...
: Table 1. Names and associated wavelengths for the seven regions of the electromagnetic spectrum [@zwinkels_encyclopedia_2020].
...
The combination of these three scattering types leads to drastic differences between the amount of solar irradiance at the top of the atmosphere and at sea level (Figure \@ref(fig:11-solar-radiation)). There are also a variety of wavelengths at which ozone, oxygen, water and carbon dioxide absorb incoming radiation, precluding entire sections of the EMS from reaching the surface. Overall, only a small portion of energy emitted from the sun reaches the Earth's surface (\~1360 W · m^-2^), depending on the time of day and year. Most energy is absorbed or scattered by particles in the Earth's atmosphere.
...
(ref:11-ems-visualization-static-caption) Online tool for visualizing the spectral signatures for different land covers using hyperspectral Hyperion satellite imagery. Data from United States Geological Survey - Licensed by U.S. Public Domain. Kenny Zhou, CC-BY-SA-4.0. Interactive tool can be viewed in the web browser version of the textbook: https://ubcemergingmedialab.github.io/geomatics-textbook/viz/emspectrum-viz/

```{r 11-ems-visualization-static, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:11-ems-visualization-static-caption)"
} else { 
  fig_cap <- paste0("Online tool for visualizing the spectral signatures for different land covers using hyperspectral Hyperion satellite imagery. Data from United States Geological Survey - Licensed by U.S. Public Domain. <a href='https://ubcemergingmedialab.github.io/geomatics-textbook/viz/emspectrum-viz/'>Click here to access the interactive tool.</a> Kenny Zhou, CC-BY-SA-4.0.")
}
knitr::include_graphics(here::here("images", "11-ems-visualization-static.png"))
```
<br/>
...
Reflectance refers to EMR that has interacted with and been reflected by a surface. It has emerged as a popular method of evaluating a variety of  environmental properties, including land use change, plant health and plant diversity [@asner_spectroscopy_2011]. Another popular normalized measure of the interaction between photons and a surface is transmittance. A photon that is transmitted has passed through the surface with which it interacted and provides insight regarding how much energy can reach other surfaces below. In a forestry context, this information can be particularly useful when determining the amount of radiation that reaches below the upper canopy (cite LAI, etc.). Absorption is a third, related measurement that refers to the amount of energy absorbed by the cells within a surface and is roughly equal to the amount of energy not captured as reflectance or transmittance (Equation \@ref(eq:absorptance)).
...
```{r 11-specsig, fig.cap = fig_cap, out.width = "200%", echo = FALSE}
    fig_cap <- paste0("Five spectral signatures of various living and non-living samples collected using an ASD FieldSpec3 Imaging Spectroradiometer. Live = live broadleaf, Dead = dead broadleaf, Fir = Douglas-fir needles on branch, Rock = rock, soil = dry humus soil. Hacker, CC-BY-SA-4.0.")
...
The radiometric resolution of a detector, then, is the number of discernible values, or segments, present between zero and maximum DN values. These segments are usually referred to as bits and can be mathematically represented as an exponent of 2 (Equation \@ref(eq:bits)). The number of bits in that a detector can resolve may also be called the **colour depth** of the image. Figure \@ref(fig:11-radiometric) clearly presents the increase in detail provided by additional bits, but recall that increasing any resolution generally increases storage and processing time. As such, it is important to select an appropriate radiometric resolution based on the needs of your study.
...
```{r 11-radiometric, fig.cap = fig_cap, echo = FALSE}
    fig_cap <- paste0("Discernable values of sensors with 1, 2 and 4 bot radiometric resolutions. Hacker, CC-BY-SA-4.0.")
...
<!--chapter:end:11-fundamentals-of-remote-sensing.Rmd-->
...
(ref:12-focus-example-humaneye-caption) Focusing lens (Vorenkamp 2015). Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/remote-sensing-systems.html#fig:12-focus_example_humaneye.

```{r 12-focus-example-humaneye, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:12-focus-example-humaneye-caption)"
  knitr::include_graphics("images/12-focus_example_humaneye-static.png")
} else { 
...
fig_cap <- paste0("Measuring the radius of curvature for a convex optical surface. Armour, CC-BY-SA-4.0")
knitr::include_graphics("images/12-ROC_convex.png")
```
<br/>

```{r 12-RoC-concave, fig.cap = fig_cap, out.width="90%", echo = FALSE}
fig_cap <- paste0("Measuring the radius of curvature for a concave optical surface. Armour, CC-BY-SA-4.0")
...
fig_cap <- paste0("Measurements in a biconvex lens. [@drbob_positive_2006], CC-BY-3.0 Unported.")
...
fig_cap <- paste0("Measurements in a biconcave lens. [@drbob_negative_2006], CC-BY-3.0 Unported.")
...
The **sensor** is the subsystem that is responsible for digitally recording the intensity of electromagnetic radiation. Sensors are  engineered in different ways to suit the type of electromagnetic radiation that needs to be recorded. However, most sensors will by comprised by three primary elements: a filter, an array of detectors, and an analog-to-digital converter (ADC). The **filter** is responsible for ensuring that only the desired wavelength of electromagnetic radiation enters the sensor. For example, in order to image only the near infrared part of the spectrum, a filter is needed to block out photons of all other wavelengths from entering the sensor. Once the desired wavelength of electromagnetic radiation is in the sensor, then the photons fall onto **detectors**, which are responsible for recording the electromagnetic radiation at a specific location in the array. Finally, the **analog-to-digital converter** is responsible for converting the photon energy into a measurable electrical charge that eventually becomes the digital number in the image for a given pixel. In summary, the photons enter the telescope or camera lens, are focused onto a plane through a lens, then filtered by wavelength, then they fall onto individual detectors before being converted to digital signals that represent numbers in a raster image. This whole process is illustrated in Figure \@ref(fig:12-lens-sensor-filter-detector-ADC) below.
...
if (knitr:::is_latex_output()) { 
  ...
  knitr::include_graphics("images/12-push-broom.gif")
}
```
</br>
...
(ref:12-whisk-broom-caption) Visualization of how a whisk broom scanner captures imagery. The dark blue squares represent the subset of the area seen by the scanner at any given time and the lighter blue squares show previously scanned areas. The size of the subset may change between a single pixel (one square) or a spotlight (multiple squares) but the motion remains the same. Armour, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/remote-sensing-systems.html#fig:12-whisk-broom.

```{r 12-whisk-broom, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:12-whisk-broom-caption)"
  knitr::include_graphics("images/12-whisk-broom-static.png")
} else { 
  fig_cap <- paste0("Visualization of how a whisk broom scanner captures imagery. The dark blue squares represent the subset of the area seen by the scanner at any given time and the lighter blue squares show previously scanned areas. The size of the subset may change between a single pixel (one square) or a spotlight (multiple squares) but the motion remains the same. Armour, CC-BY-SA-4.0.")
  knitr::include_graphics("images/12-whisk-broom.gif")
}
```
</br>
...
(ref:12-aerial-good-caption) How aerial imagery should be taken. Armour, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/remote-sensing-systems.html#12-aerial-good.

```{r 12-aerial-good, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:12-aerial-good-caption)"
knitr::include_graphics("images/12-aerial-good-static.png")
} else { 
  fig_cap <- paste0("How aerial imagery should be taken. Armour, CC-BY-SA-4.0.")
  knitr::include_graphics("images/12-aerial-good.gif")
}
```
<br/>

(ref:12-aerial-bad-caption) How aerial imagery should not be taken. Armour, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/remote-sensing-systems.html#12-aerial-bad.

```{r 12-aerial-bad, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:12-aerial-bad-caption)"
  knitr::include_graphics("images/12-aerial-bad-static.png")
} else { 
  fig_cap <- paste0("How aerial imagery should not be taken. Armour, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Visualization of a hemispherical perspective. Armour, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Hemispherical photo taken in the Bavarian forest. [@wegmann_hemispherical_2011], CC-BY-3.0.")
...
fig_cap <- paste0("Picture of Newfoundland, Canada, taken by David Saint-Jacques during his space mission. [@canadian_space_agency_newfoundland_2019], CC-BY-3.0.")
...
```{r 12-platforms, fig.cap = fig_cap, out.width="50%", echo = FALSE}
...
knitr::include_graphics("images/12-platforms.png"))
```
<br/>
...
knitr::include_graphics("images/12-google-street-view-car.png"))
```
<br/>
...
if (knitr:::is_latex_output()) { 
  knitr::include_graphics("images/12-phenological-camera-sequence-static.png")
} else { 
  knitr::include_graphics("images/12-phenological-camera-sequence.gif")
}
```
...
if (knitr:::is_latex_output()) { 
  knitr::include_graphics("images/12-mountain-legacy-project-static.png")
} else { 
  knitr::include_graphics("images/12-mountain-legacy-project.gif")
}
```
<br/>
...
(ref:12-sunsynchronous-geosynchronous-orbits) Comparing Sun-synchronous and geosynchronous orbits. The yellow area shows the portion of Earth's surface that is visible during a single orbit. Notice that Sun-synchronous orbit observes at a consistent local time while the geosynchronous orbit observes a constant location. Credit: [NOAA/JPL-Caltech](https://scijinks.gov/about/. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/remote-sensing-systems.html#fig:12-sunsynchronous-geosynchronous-orbits.

```{r 12-sunsynchronous-geosynchronous-orbits, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:12-sunsynchronous-geosynchronous-orbits-caption)"
  knitr::include_graphics("images/12-sunsynchronous-geosynchronous-orbits-static.png")
} else { 
  ...
  knitr::include_graphics("images/12-sunsynchronous-geosynchronous-orbits.gif")
}
```
<br/>
...
<!--chapter:end:12-remote-sensing-systems.Rmd-->
...
if (knitr:::is_latex_output()) { 
  knitr::include_graphics("images/13-Beaufort-Sea-Ice-static.png")
} else { 
...
(ref:13-dnbr-leaflet-caption) Classified Burn Severity images for the Little Bobtail Lake wildfire from 2015 to 2020. Murray, CC-BY-SA-4.0. Data from Natural Resources Canada and licensed under the Open Government Licence - Canada: https://www.nrcan.gc.ca/terms-and-conditions/10847. Interactive web map can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/image-processing-and-analysis.html#13-dnbr-leaflet.

```{r 13-dnbr-leaflet, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:13-dnbr-leaflet-caption)"
  knitr::include_graphics("images/13-dnbr-leaflet.png")
} else { 
  fig_cap <- paste0("Classified Burn Severity images for the Little Bobtail Lake wildfire from 2015 to 2020. Data from @natural_resources_canada_natural_nodate and licensed under the <a href='https://www.nrcan.gc.ca/terms-and-conditions/10847'>Open Government Licence - Canada </a>. Murray, CC-BY-SA-4.0.")
  dnbrMap
}
```
<br/>
...
<!--chapter:end:13-image-processing-and-analysis.Rmd-->
...
<!--chapter:end:14-global-navigation-satellite-systems.Rmd-->
...
```{r echo=FALSE}
yml_content <- yaml::read_yaml("chapterauthors.yml")
author <- yml_content[["lidarAcquisitionAndAnalysis"]][["author"]]
coauthor <- yml_content[["lidarAcquisitionAndAnalysis"]][["coauthor"]]
```
...
**LiDAR** stands for **Light Detection And Ranging** (sometimes written as lidar, or LIDAR), and is an active remote sensing technology. A laser scanner and time of flight principles are used to collect three dimensional (3D) data. LiDAR systems are made up of three components; a laser-scanning device, an accurate global navigation satellite system (GNSS), and an inertial measurement unit (IMU). The laser-scanning device emits pulses of light and measures the time it takes for energy to be reflected to the device. The GNSS receiver allows the position of the laser to be determined in space, while the IMU records the orientation of the laser (i.e. roll, pitch, and yaw [@white_best_2013]). Figure \@ref(fig:15-LiDAR-System) illustrates all of the necessary components of a LiDAR system. When discussing LiDAR in an airborne context (i.e. the unit is being flown), we can call it airborne laser scanning (ALS), and if the unit is in a fixed position on the ground it is called terrestrial laser scanning (TLS).

```{r 15-LiDAR-System, fig.cap = fig_cap, out.width="60%", echo = FALSE}
    fig_cap <- paste0("Overview of the components of a LiDAR System. As is common for large scale data acquisition, the laser scanning is placed on board an aircarft and scans the Earth below it. The aircraft has on on board GPS/GNSS, as well as an intertial measurement unit (IMU). A GPS base station can be used to post-process data and increase spatial accuracy.('LiDAR-i lend', @marek9134_lidar-i_2012, CC-BY-SA-3.0).")
    knitr::include_graphics("images/15-LiDAR-System.png")
```
...
fig_cap <- paste0("Concept of LiDAR. A light signal is emitted by the scanner and reflected off the target. 'Concept of LiDAR', @cartographer3d_concept_2021, CC-BY-SA-4.0.")
knitr::include_graphics("images/15-Concept-of-LiDAR.png")
```
...
The most common file format for LiDAR files is called the LAS file format (.las). This file format was originally designed for 3D point cloud data, and is a free alternative to proprietary systems or a generic ASCII file interchange system. The main benefits of this file format are that it is relatively quick, can be used by any system, and stores information specific to the nature of LiDAR data without being overly complex [@american_society_for_photogrammetry__remote_sensing_laser_2019]. More information regarding the file type specifications can be found [at their website](https://www.asprs.org/divisions-committees/lidar-division/laser-las-file-format-exchange-activities){target="_blank"}.

```{r 15-las-denoise, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap <- paste0("An example of a typical LiDAR point cloud containing 3.14 million points at approximately 80 points·m^-2^. The point cloud shows a 200 x 200 meter portion of Pacific Spirit Regional Park in Vancouver, and is part of the [2018 City of Vancouver dataset] (https://opendata.vancouver.ca/explore/dataset/lidar-2018/). Du Toit, CC-BY-SA-4.0.")
knitr::include_graphics("images/15-las-denoise.png")
```

## LiDAR History and Use

LiDAR for forestry and ecology has been used to monitor vegetation structure, stream properties, and topography among other uses. The earliest versions of LiDAR were known as profiling systems and first investigated in the 1960's [@nelson_how_2013]. A major limitation of early LiDAR profiling systems were that the system was locked in a near-nadir position (i.e. along-track path), which meant that only transects could be collected, as opposed to a larger swath width as with modern systems [@nelson_how_2013; @lim_lidar_2003]. Technological advancements meant that early scanning LiDAR systems became more common in the 1980s, although point densities (measured as points per square meter, points·m^-2^) were low. Densities of 1-5 points·m^-2^ limited researchers to area based measurements of forest volume and biomass [@nelson_how_2013]. These limitations were primarily due to the LiDAR sensor, as well as data storage difficulties. Technological improvements such as increased pulse rates and smaller footprints (leading to increased point density), and increased storage capacity have allowed researchers to look as individual trees as well as whole forests [@jakubowski_delineating_2013]. Previously limited to static LiDAR sensors, ALS point cloud densities of 1000 points·m^-2^ are now entirely feasible. Figure \@ref(fig:15-high-low-point-density) shows two different point clouds densities, and how difficult it would be to delineate individual tree crowns at low point densities.

```{r 15-high-low-point-density, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap <- paste0("A cross section of a high density point cloud (A, 80 points·m^-2^), and a lower density point cloud (B, 1 point·m^-2^). Du Toit, CC-BY-SA-4.0.")
knitr::include_graphics("images/15-high-low-point-density.png")
```

Unlike passive remote sensing technologies, LiDAR has the advantage of being able penetrate forest canopies; this means that we are able to detect the ground, and also characterize both vertical and horizontal vegetation structure. Since the data is extremely accurate, surface models developed using LiDAR are used in many fields where human-made and natural environments need to be mapped. LiDAR derivatives such as surface models are used in hazard assessment (see here: @jaboyedoff_use_2012), forestry (read more here: @goodbody_airborne_2021), wet area mapping (here: @eash_stream-channel_2018 and @zurqani_evaluating_2020), geologic/geomorphological mapping, and agriculture. In this chapter, we will discuss the use of LiDAR primarily in a forestry context, where raw data and derivatives are used for volume and biomass estimation, as well as individual tree crown analyses.

## Components of a LiDAR System

### Lasers

**Lasers** are a very important component of LiDAR systems. Here we will discuss some basic concepts, as well as the technical parameters that are important when interpreting LiDAR data. LiDAR lasers are typically beams of **near-infrared** light (800 -- 1,550 nm, typically 1,064 nm), and are used because these wavelengths are considered eye safe [@white_best_2013]. Green wavelengths are used for bathymetric LiDAR (i.e. water penetrating LiDAR), but are less common, and not typically used on land [@uf_geomatics_-_fort_lauderdale_lidar_2016-2]. Laser scanners use rotating or oscillating mirrors in order to 'scan' a scene in multiple dimensions. When these scanners are placed on a moving platform (e.g. a plane), we can cover large areas. LiDAR sensors scan in a variety of ways: whisk broom, rotation mirror line, and push broom scanners being the most common.

**Beam divergence** is a property that refers to how wide the light beam becomes when it intercepts an object, and can be used to differentiate LiDAR instruments. Small-footprint LiDAR describes beam diameters intercepting the surface at \< 1 m, while large-footprint intercepts the surface at around 5 - 25 m [@lim_lidar_2003]. Small-footprint LiDAR is primarily what is used in forest inventory and ecology studies, and has high accuracy, as well as the ability to produce high sampling densities. For these studies, beam divergence is typically 0.15 -- 2.0 mrad [@white_best_2013]. The concept is illustrated in Figure \@ref(fig:15-Beam-Divergence), and the equation is shown below in \@ref(eq:footprint), where $D$ is the footprint diameter, $h$ is the flight height (m), and $\gamma$ is the laser beam divergence (mrad).
...
fig_cap <- paste0("An example of a LiDAR Unit. 'LiDAR machine', @the_center_for_international_forestry_research_cifor_lidar_2014, CC-BY-NC-ND-2.0.")
knitr::include_graphics("images/15-LiDAR-Unit.png")
```
...
fig_cap <- paste0("An on board GNSS is required toprovide an accurate location of the LiDAR scanner in space. ('A key component of a lidar system is a GPS', @neon_education_key_2014, CC-BY-NC-SA-2.0).")
knitr::include_graphics("images/15-XYZ-coordinates.png")
```
...
```{r 15-LiDAR-on-Drone, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap <- paste("Drone mounted LiDAR. This technology is rapidly developing, with many companies working towards creating units capable of acquiring high density datasets over large areas. 'LiDARUSA Snoopy 120 LiDAR', @mc_clapurhands_lidarusa_2019,  CC-BY-SA-4.0.")
knitr::include_graphics("images/15-LiDAR-on-Drone.png")
```
...
fig_cap <- paste0("LiDAR Platforms 1: A terrestrial laser scanner (TLS). 'Lidar P1270901.png', @monniaux_lidar_2007, CC-BY-SA-3.0. 2: A hand-held moblie laser scanner (MLS). Du Toit, CC-BY-SA-4.0. 3: Car mounted MLS. '3D mobile mapping unit', @oregon_department_of_transportation_3d_2016, CC-BY-2.0'. 4: GEDI, a spaceborne LiDAR sensor. 'A New Hope: GEDI to Yield 3D Forest Carbon Map', @neon_education_key_2014, CC-BY-2.0.")
knitr::include_graphics("images/15-LiDAR-Platforms.png")
```

### Discrete Return

**Discrete return** LiDAR is the most common type of LiDAR. Here, the reflected energy from a pulse is converted into return targets referenced in time and space [@white_best_2013]. Discrete return systems can often record multiple returns (often 5 or more), as long as the energy from each return meets the energy threshold to be classified as a return. Returns are classified per pulse, by the arrival sequence in which the return signals are detected by the sensor [@uf_geomatics_-_fort_lauderdale_lidar_2016]. In the case where a laser pulse intercepts a solid object (such as a building), only a single return would occur; in contrast if the laser pulse can penetrate through the object (e.g. the forest), some of the energy will be returned to the instrument where the pulse intercepts stems/branches/leaves [@white_best_2013]. Figure \@ref(fig:15-LiDAR-Discrete-Full-Waveform) shows an example of this; a discrete return system would interpret the returned echo as two returns, as there are two peaks in the returned echo corresponding to a canopy and a ground hit. In a forested landscape, first returns often represent the upper canopy, while last returns correspond to the ground or objects near the ground [@white_best_2013].

### Full Waveform

**Full waveform** LiDAR systems record reflected energy from each laser pulse as a continuous signal [@white_best_2013].Traditionally, discrete return systems would only digitally store the discrete returns, without recording the complete analog return [@uf_geomatics_-_fort_lauderdale_lidar_2016]. In contrast, full waveform systems digitize the entire analog signal at high sampling rates [@uf_geomatics_-_fort_lauderdale_lidar_2016]. This can be seen on the right hand side of Figure \@ref(fig:15-LiDAR-Discrete-Full-Waveform). This type of LiDAR creates much larger file sizes which were an issue in the past, but with modern storage capabilities it is becoming a viable alternative to discrete return LiDAR.

```{r 15-LiDAR-Discrete-Full-Waveform, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap <- paste0("Differents in the response between discrete echo and full waveform LiDAR. 'Airborne Laser Scanning Discrete Echo and Full Waveform signal comparison', @beck_airborne_2012, CC-BY-SA-3.0.")
knitr::include_graphics("images/15-LiDAR-Discrete-Full-Waveform.png")
```
...
(ref:15-lidar-visualization-static-caption) Online tool for visualizing and exploring the properties of a small LiDAR point cloud. Juno Yoon, CC-BY-SA-4.0. Data from City of Vancouver and licensed under the Open Government License - Vancouver: https://opendata.vancouver.ca/pages/licence/. Interactive tool can be viewed in the web browser version of the textbook: https://ubcemergingmedialab.github.io/geomatics-textbook/viz/lidar-viz/.

```{r 15-lidar-visualization-static, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { 
  fig_cap <- "(ref:15-lidar-visualization-static-caption)"
} else { 
  fig_cap <- paste0("Online tool for visualizing and exploring the properties of a small LiDAR point cloud. Data from City of Vancouver and licensed under the <a href='",covlicence,"'>Open Government License - Vancouver</a>. <a href='https://ubcemergingmedialab.github.io/geomatics-textbook/viz/lidar-viz/'>Click here to access the interactive tool.</a> Juno Yoon, CC-BY-SA-4.0.")
}
knitr::include_graphics(here::here("images", "15-lidar-visualization-static.PNG"))
```
<br/>

## Emerging Technology

**Single Photon LiDAR** is an emerging technology that allows wall-to-wall mapping detecting photons reflected off of surfaces more efficiently than traditional LiDAR sensors [@swatantran_rapid_2016]. While traditional LiDAR systems emit high energy laser beams, single photon systems transmit shorter, lower energy pulses [@swatantran_rapid_2016]. These systems use a green laser (532 nm), and due to the high efficiency nature of the technology can acquire high density point clouds (12 - 30 points·m^-2^) up to 30 times faster than traditional systems, and operate at higher altitudes [@swatantran_rapid_2016]. The system is also capable of penetration semi-porous obscurations such as vegetation, ground fog and thin clouds during daytime and night-time operations [@swatantran_rapid_2016].

**Multispectral LiDAR** units such as the Optech Titan sensor operate three distinct wavelengths of 1,550 nm, 1,064 nm, and 532 nm [@morsy_multispectral_2017]. This technology enables 3D spectral information to be captured; for example, NDVI can be calculated for the 3D point cloud. Combining multispectral LiDAR at three wavelengths allows for higher reliability and accuracy compared to single-wavelength LiDAR [@morsy_multispectral_2017], however this technology is not yet widespread.
...
Digital Elevation Models (DEM) are raster products that represent elevation of the Earth's surface above a reference vertical datum such as sea level [@white_best_2013]. Using a classified point cloud, we can isolate the points that are categorized as ground (`Classification == 2`) and create a raster that represents the ground (see Figure \@ref(fig:15-las-dem) below). DEMs are also called digital terrain models (DTMs), although a DTM can include vector features that represent natural features such as river channels, cliffs, ridges, and peaks. DEMs have a variety of uses, from terrain modeling for watersheds, to road design [@roussel_lidr_2021]. In addition to being used on their own, DEMs can be used to normalize point clouds; this is the process of subtracting the ground elevation from a point cloud in order to place the points on a flat plane, so that each point represents the height above the ground, not absolute height. To create a DEM, various interpolation techniques are available, but common algorithms include inverse distance weighting (IDW), kriging, and k-nearest neighbour (KNN).
...
fig_cap <- paste0("A typical LiDAR processing workflow. After acquiring data, pre-processing is necessary to clean and normalize the point cloud. After this a CHM can be created to detect tree tops, before segmentating the point cloud based on these points. Du Toit, CC-BY-SA-4.0." )
knitr::include_graphics("images/15-Processing-Flowchart.png")
```
...
fig_cap <- paste0("Example of tree tops detected using the `find_trees` function. Du Toit, CC-BY-SA-4.0." )
knitr::include_graphics("images/15-las-treetops.png")
```
...
```{r 15-multipath-error, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap <- paste0("Example of multipath errors that can occur. The green dot is where our point is actually located in space, while the blue point shows where the point is placed in the point cloud. The red line shows the path of the reflected pulse. Du Toit, CC-BY-SA-4.0." )
knitr::include_graphics("images/15-Multipath-Error.png")
```

## Software and Analysis Tools

As LiDAR acquisition becomes cheaper, more tools are becoming available to do analysis. In this chapter, we use [lidR](https://cran.r-project.org/web/packages/lidR/){target="_blank"}; a free and open source R package that can be used for the entire process of analyzing a point cloud. Several other options exist, such as [A Shiny-based Application for Extracting Forest Information from LiDAR data](https://github.com/carlos-alberto-silva/weblidar-treetop){target="_blank"} (also in R), the [Digital-Forestry-Toolbox](http://mparkan.github.io/Digital-Forestry-Toolbox/){target="_blank"} which is available for MATLAB, [FUSION](http://forsys.cfr.washington.edu/fusion/fusion_overview.html){target="_blank"}, a software developed by the USDA Forest Service, and finally [QGIS](https://www.qgis.org/en/site/){target="_blank"}, a free GIS. Paid software is also frequently used in LiDAR processing (examples include [ArcGIS Pro](https://www.esri.com/en-us/home){target="_blank"}, and [LAStools](https://rapidlasso.com/lastools/){target="_blank"}), although the cost can be prohibitive. Finally, the open source software [CloudCompare](https://www.danielgm.net/cc/){target="_blank"} can be incredibly useful for both viewing and manually clipping/editing point clouds.

## Case Study: Creating LiDAR Metrics from a Raw Point Cloud

For this case study, we will be using a clipped .las file from the 2018 open [LiDAR dataset](https://opendata.vancouver.ca/explore/dataset/lidar-2018/information/){target="_blank"} of the City of Vancouver and UBC Endowment Lands in British Columbia (we randomly selected a 200 x 200 m portion of the '4840E_54550N' tile using CloudCompare), and the `lidR` package in R [@city_of_vancouver_lidar_nodate],[@roussel_lidr_2020]. The script to process this data is included in the data folder of the GitHub repository, and you can use the `lidR` [book](https://r-lidar.github.io/lidRbook/){target="_blank"} to get a more in depth understanding of the functions we apply below [@roussel_lidr_2021].
...
fig_cap <- paste0("A normalized point cloud. Du Toit, CC-BY-SA-4.0." )
knitr::include_graphics("images/15-las-normalize.png")
```
...
fig_cap <- paste0("A segmented point cloud using the algorithm based on @dalponte_tree-centric_2016. Du Toit, CC-BY-SA-4.0.")
knitr::include_graphics("images/15-las-segmented.png")
```
...
```{r 15-files, echo=FALSE, warning=FALSE, include =FALSE, cache=TRUE}
# Load files
...
raster::crs(r_hmean) <- "+proj=utm +zone=10 +datum=NAD83 +units=m"
raster::crs(r_hmax) <- "+proj=utm +zone=10 +datum=NAD83 +units=m"
raster::crs(r_hsd) <- "+proj=utm +zone=10 +datum=NAD83 +units=m"
raster::crs(r_hzq15) <- "+proj=utm +zone=10 +datum=NAD83 +units=m"
raster::crs(r_hpabove2) <- "+proj=utm +zone=10 +datum=NAD83 +units=m"
...
```{r 15-map, width="100%", echo=FALSE, fig.cap = fig_cap}
fig_cap <- paste0("Explore different area-based metrics produced in the case study above. Hover over tile icon (top left) to turn different layers on or off. Du Toit, CC-BY-SA-4.0")
knitr::opts_chunk$set(fig.width = 12, fig.height = 12) #make the interactive maps bigger/smaller across the document
if (knitr:::is_latex_output()) {
knitr::include_graphics(here::here("images", "15-map.PNG"))
} else {
m
}
```
...
-   A best practices guide for generating forest inventory attributes from airborne laser scanning data using an area-based approach. Information Report FI-X-010 - @white_best_2013
-   Airborne laser scanning for quantifying criteria and indicators of sustainable forest management in Canada - @goodbody_airborne_2021
-   Use of LIDAR in landslide investigations: A review - @jaboyedoff_use_2012

```{r, include=FALSE, echo=FALSE}
knitr::write_bib(c(
  .packages(), 'bookdown', 'knitr', 'rmarkdown', 'htmlwidgets', 'webshot', 'DT',
  'miniUI', 'tufte', 'servr', 'citr', 'rticles', 'leaflet', 'raster', 'sf', 'sp',
  'htmlwidgets', 'rgdal', 'leafem', 'geojsonio', 'lidR'), 'packages.bib')
```
